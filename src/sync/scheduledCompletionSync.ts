...
import { getText, getNumber, hasFiles } from "../notion/properties.js"
...
export async function findCandidates(notion: Client): Promise<CandidatePage[]> {
  ...
  do {
    ...
    for (const result of page.results) {
      ...
      candidates.push({ pageId: result.id, sendId, formId, receiverMetaId })
    }
...
worker.sync("widsignScheduledCompletionSync", {
  ...
  execute: async (_state, { notion }) => {
    ...
    if (toUpdate.length > MAX_UPDATES_PER_RUN) {
      console.error(
        `[widsignScheduledCompletionSync] 이번 실행에서 수정 예정 건수(${toUpdate.length})가 ` +
          `한도(WIDSIGN_MAX_UPDATES_PER_RUN=${MAX_UPDATES_PER_RUN})를 초과해 아무 것도 수정하지 않습니다. ` +
          "예상치 못하게 많은 계약이 한꺼번에 완료 처리 대상이 됐다면 원인을 먼저 확인하세요.",
      )
    } else if (DRY_RUN) {
      console.log(
